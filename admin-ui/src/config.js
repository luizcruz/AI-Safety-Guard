...
function loadConfig(environment = process.env) {
  ...
  const adminKey = String(environment.AI_SAFETY_ADMIN_KEY || "");
  if (adminKey.length < 16) throw new Error("AI_SAFETY_ADMIN_KEY deve ter pelo menos 16 caracteres");
  if (adminKey === apiToken) throw new Error("AI_SAFETY_ADMIN_KEY deve ser diferente de AI_SAFETY_API_TOKEN");
  ...
  return Object.freeze({ apiToken, adminKey, apiUrl: apiUrl.origin, port });
}
